...
export function calculateCandidateScore(candidate, vacancy) {
    let score = 0;
    const matchedRequiredSkills = vacancy.requiredSkills.filter(requiredSkill => candidate.skills.some(candidateSkill => candidateSkill.toLowerCase() === requiredSkill.toLowerCase()));
    ...
    const matchedPreferredSkills = vacancy.preferredSkills.filter(preferredSkill => candidate.skills.some(candidateSkill => candidateSkill.toLowerCase() === preferredSkill.toLowerCase()));
    score += Math.min(matchedPreferredSkills.length * 10, 20);
    if (candidate.yearsOfExperience >= vacancy.minYearsExperience &&
        candidate.yearsOfExperience <= vacancy.maxYearsExperience) {
        score += 20;
    }
    else {
        const difference = candidate.yearsOfExperience < vacancy.minYearsExperience
    ...
    if (candidate.expectedSalary >= vacancy.salaryRangeMin &&
        candidate.expectedSalary <= vacancy.salaryRangeMax) {
        score += 10;
    }
    else if (candidate.expectedSalary > vacancy.salaryRangeMax &&
