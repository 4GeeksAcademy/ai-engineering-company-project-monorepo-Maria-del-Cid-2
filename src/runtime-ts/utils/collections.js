...
export function filterCandidatesBySkills(candidates, requiredSkills) {
    return candidates.filter(candidate => requiredSkills.every(requiredSkill => candidate.skills.some(candidateSkill => candidateSkill.toLowerCase() === requiredSkill.toLowerCase())));
}
